...
(function (Builder, $, undefined) {
    ...
    function SetupUnderlineWords() {
        $(".underline-words").each(function () {
            var wordList = $(this).text().split(",").filter(function (el) {
                return el.length > 0;
            });
            var id = $(this).attr("id");
    ...
    */
    function SetupFindReplaceWords() {
        ...
        $(".find-replace-words").each(function () {
            ...
            function AddWord(tempDiv, word, elementValue) {
                // const exampleWord = word.startsWith('<span>') ? "was" : "";
                ...
                // const hiddenClass = exampleWord ? " " : "hidden"
                ...
                var input = $('<input type="text" id="' + id + '_' + i + '" class="inline-text" ' + elementValue + ' />').data({
                    "tempElement": tempElement,
                    "word": wordElm,
                    "storageId": opg + "_" + id + '_' + i,
                    "i": i,
                    "hidden": hidden
            ...
            // ordet "were"
            // classen
            // span hidden kig ind på for "was" ind i boksen.
            // ".input-find-replace-text"

            // $('.find-replace-words').each(function(index, me) {
            //     var words = $(me).text().trim().split(' ');
            //     words[2] = '<span class="aspanclass2">' + words[0] + '</span>';
                ...
                var ckEditor = ClassicEditor
                    ...
                    .catch(error => {
                        ...
                        if (typeof wordList !== "undefined") {
                            ...
                            editor.editing.view.document.on("keyup", function (e) {
                                var text = concatinateReachText(e.source.roots._items[0]._children);
                                CheckWordList(wordList, text);
                            });
                        }
                    });
            }
    ...
    function TrueSave() {
        ...
        if ($("#tech-info").length == 0) {
            techinfo = $('<div id="tech-info"></div>').appendTo($("[data-tech-info]"));
        }
        else {
            techinfo = $("#tech-info");
        }
    ...
    function concatinateReachText(itemsList) {
        var concatinated = "";
        for (var i = 0; i < itemsList.length; i++) {
            concatinated += itemsList[i]._children[0].data;
        }
        return concatinated;
    }
    ...
    function CheckWordList(item) {
        ...
        var extraParameters = arguments[1];

        if (wordlist.length > 0) {
            var answerList = $('[data-wordlist="' + item + '"]')
                .map(function () {
                    if (typeof $(this).attr("data-replace") !== "undefined") {
                        if ($(this).find("input").length > 0) {
                            return $(this).find("input").val().toLowerCase();
                        }
                    } else if (extraParameters !== undefined) {
                        return extraParameters.toLowerCase();
                    } else {
                ...
                if (inputText !== "") {
                    if (type == "strict") {
                        for (var i = 0; i < answerList.length; i++) {
                            if (
                                answerList[i].toLowerCase() == inputText.toLowerCase() ||
                                answerList[i].toLowerCase() == inputText2.toLowerCase()
                            ) {
                                found = true;
                                break;
                            }
